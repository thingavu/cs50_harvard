...
class Sentence():
    ...
    def known_mines(self):
        ...
        """
        if (self.count > 0) & (len(self.cells) == self.count):
            return self.cells
        else:
            return set()
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for row in range(i - 1, i + 2):
            for col in range(j - 1, j + 2):
                if (row >= 0) & (row < self.height) & (col >= 0) & (col < self.width) & ((row, col) != cell):
                    if ((row, col) not in self.safes) & ((row, col) not in self.mines):
                        neighbors.append((row, col))
                    if (row, col) in self.mines:
                        count_mines -= 1

        new_sentence = Sentence(neighbors, count_mines)
        self.knowledge.append(new_sentence)                        

        self.marks_cells()
        self.update_knowledge(new_sentence)

    def marks_cells(self):
        for sentence in self.knowledge:            
    ...
    def update_knowledge(self, new_sentence):
        for sentence in self.knowledge:
            if (sentence.count > 0) & (new_sentence.count > 0) & \
                (new_sentence.cells.issubset(sentence.cells)) & (new_sentence != sentence):
    ...
    def make_random_move(self):
        ...
        for i in range(self.height):
            for j in range(self.width):
                if ((i, j) not in self.mines) & ((i, j) not in self.moves_made):
                    moves_available.add((i, j))        
        if len(moves_available) == 0:
            return None
        return random.choice(list(moves_available))    
